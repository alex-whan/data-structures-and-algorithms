...
const getStatName = (arr, minBaseStat) => {
  let filteredArray = arr.filter(value => value.baseStat > minBaseStat);
  return filteredArray.map(value => value.stat.name);
};
...
const getCharactersWithoutChildren = arr => {
  const filteredArr = arr.filter(person => !person.children);
  return filteredArr;
};
...
const evenOddNumericValues = arr => {
  ...
  filtered.map(number => results.push(number % 2 === 0 ? 'even' : 'odd'));
  return results;
};
