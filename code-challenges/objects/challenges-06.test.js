...
const hasChildrenValues = (arr, character) => {
  ...
  const person = Object.values(personToCheck);
  if (person.length > 0) {
    return person[0].children.length > 0 ? true : false;
  } else {
    return false;
  }
};
...
const totalCharacters = arr => {
  ...
  for (let character of arr) {
    total += 1;
    character.spouse ? (total += 1) : (total += 0);
...
const houseSurvivors = arr => {
  ...
  for (let family of arr) {
    const spouse =
      family.spouse && !deceasedSpouses.includes(family.spouse) ? 1 : 0;
    const children = family.children.length;
    const house = {
      house: family.house,
      members: 1 + spouse + children,
